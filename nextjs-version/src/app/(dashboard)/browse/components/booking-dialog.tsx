...
export function BookingDialog({ slot, open, onOpenChange, onConfirm }: BookingDialogProps) {
  ...
  const handleConfirm = async () => {
    try {
      ...
      const booking = await bookingService.createBooking(s.slot_id)
      // 2. Initiate Chapa payment — redirects student to checkout
      const { checkout_url } = await paymentService.initiate(booking.booking_id)
      // Redirect to Chapa — student leaves the app
          ...
          <>
            ...
            <DialogFooter className="gap-2 sm:gap-0">
              ...
              <Button onClick={handleConfirm} disabled={confirming}>
                {confirming ? "Redirecting to payment…" : "Pay & Confirm Booking"}
              </Button>
            </DialogFooter>
          </>
        )}
