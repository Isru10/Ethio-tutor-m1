...
export function SlotCard({ slot, onBook }: SlotCardProps) {
  ...
  const formattedDate = sessionDate.toLocaleDateString("en-ET", {
    weekday: "long", day: "numeric", month: "short",
  })
  const formattedStart = sessionDate.toLocaleTimeString("en-ET", {
    hour: "2-digit", minute: "2-digit",
  })
  const endDate = new Date(`${s.slot_date}T${s.end_time}`)
  const formattedEnd = endDate.toLocaleTimeString("en-ET", {
    hour: "2-digit", minute: "2-digit",
  })
    ...
    <Card className={`flex flex-col transition-shadow duration-200 hover:shadow-md border ${isFull ? "opacity-60" : ""}`}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                <span className="text-xs font-semibold">{teacher_profile.average_rating.toFixed(1)}</span>
              </div>
            </div>
      ...
      <CardContent className="pb-3 space-y-3">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="font-semibold">{teacher_profile.hourly_rate} ETB</span>
