...
export function HeatmapCalendar({ data, className }: HeatmapCalendarProps) {
  // Generate levels based on intensity (0-4)
  ...
  return (
    <TooltipProvider>
      <div className={cn("grid grid-cols-[repeat(20,minmax(0,1fr))] gap-1.5", className)}>
        {data.map((item, idx) => (
          <Tooltip key={idx}>
            <TooltipTrigger asChild>
              <div 
                className={cn(
                  "h-3 w-3 rounded-sm transition-all hover:ring-2 hover:ring-primary/50 cursor-help",
                  getLevel(item.intensity)
                )}
              />
            </TooltipTrigger>
    ...
    <Card className={cn("overflow-hidden border-border bg-card shadow-sm", className)}>
      <CardHeader className="pb-3 flex flex-row items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col items-end">
            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Best</span>
