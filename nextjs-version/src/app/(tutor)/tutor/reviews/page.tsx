...
interface ReviewRow {
  ...
  booking: { slot: { subject: { name: string } } }
}
    ...
    <>
      ...
      <div className="px-4 lg:px-6 grid gap-4 md:grid-cols-3">
        <Card className="flex flex-col items-center justify-center py-8 gap-3">
          <div className="text-6xl font-black">{loading ? "–" : avgRating.toFixed(1)}</div>
              ...
              : distribution.map(({ star, count, pct }) => (
                <div key={star} className="flex items-center gap-3 text-sm">
                  <div className="flex items-center gap-1 w-10 shrink-0">
                    <Star className="size-3.5 fill-amber-400 text-amber-400" /><span>{star}</span>
                  </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {loading
              ...
              : rows.map(r => {
                const studentName = r.student?.user?.name ?? "Student"
                const subject     = r.booking?.slot?.subject?.name ?? "Session"
                return (
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {subject} · {new Date(r.created_at).toLocaleDateString("en-ET",{month:"short",day:"numeric",year:"numeric"})}
                          </p>
                        </div>
                      </div>
