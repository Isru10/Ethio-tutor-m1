...
import { BookCheck, TrendingUp, Clock, CheckCircle2, Users, ChevronRight, Video, Loader2 } from "lucide-react"
...
export default function TutorBookingsPage() {
  ...
  useEffect(() => {
    ...
    bookingService.getTutorBookings()
      .then(data => { setRawBookings(data); setLoading(false) })
  ...
  const slotGroups = useMemo<SlotGroup[]>(() => {
    ...
    for (const b of rawBookings) {
      const sid = b.slot.slot_id ?? b.slot_id as any
      if (!map.has(sid)) {
        map.set(sid, {
          slot_id:            sid,
  ...
  const stats = useMemo(() => ({
    ...
    confirmed: slotGroups.filter(g => g.overallStatus === "confirmed" || g.overallStatus === "live").length,
    completed: slotGroups.filter(g => g.overallStatus === "completed").length,
  }), [slotGroups])
  ...
  return (
    <>
      <div className="px-4 lg:px-6">
        ...
        <p className="text-muted-foreground text-sm">Each row is a session slot. Click a row to see enrolled students.</p>
      </div>

      {/* Stats */}
      <div className="px-4 lg:px-6 grid gap-4 grid-cols-2 md:grid-cols-4">
        {[
          { label: "Total Slots",  value: stats.total,     icon: BookCheck,    color: "text-foreground" },
      ...
      <div className="px-4 lg:px-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Click any row to view enrolled students for that session.</CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
                ...
                <Table>
                  ...
                  <TableBody>
                    {slotGroups.length ? slotGroups.map(g => {
                      ...
                      const totalStudents = g.bookings.filter(b => b.status !== "cancelled").length

                      // Actual taught duration if session has started
                      let taughtLabel = `${durationMins} min`
                      if (g.session?.start_time && g.overallStatus === "completed") {
                        const actualEnd = new Date()
                        const actualStart = new Date(g.session.start_time)
                        const actual = Math.round((actualEnd.getTime() - actualStart.getTime()) / 60000)
                      ...
                      return (
                        <TableRow
                          ...
                          onClick={() => router.push(`/tutor/bookings/${g.slot_id}`)}
                        >
                          <TableCell>
                            <div className="flex items-center gap-2">
                              <Badge variant="default" className="text-xs">{g.subject}</Badge>
                            </div>
                          </TableCell>
                          <TableCell>
                            <div className="text-sm font-medium">
                              {startDt.toLocaleDateString("en-ET", { weekday: "short", day: "numeric", month: "short" })}
                            </div>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-1.5 text-sm">
                              ...
                              <span className="text-muted-foreground">/ {g.max_students}</span>
                            </div>
                            {g.pendingCount > 0 && (
                              <p className="text-xs text-amber-600 mt-0.5">{g.pendingCount} pending payment</p>
                            )}
                          </TableCell>
                          <TableCell>
                            <span className="text-sm text-muted-foreground">{taughtLabel}</span>
                          </TableCell>
                          ...
                          <TableCell onClick={e => e.stopPropagation()}>
                            {g.overallStatus === "confirmed" && g.startableBookingId && (
                              <Button
                                size="sm"
                                className="h-7 text-xs gap-1.5 bg-green-600 hover:bg-green-700 text-white"
                                disabled={isStarting}
                                onClick={() => handleStartSession(g.startableBookingId!, g.slot_id)}
                              >
                                {isStarting ? <Loader2 className="size-3 animate-spin" /> : <Video className="size-3" />}
                            ...
                            {g.overallStatus === "live" && (
                              <Button
                                ...
                                onClick={() => router.push(`/room/${g.session?.session_id}`)}
                              >
                                <Video className="size-3" /> Rejoin
                              </Button>
                            )}
                            {(g.overallStatus === "pending" || g.overallStatus === "completed") && (
                              <Button
